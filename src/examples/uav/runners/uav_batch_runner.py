...
class UAVBatchRunner(GenericBatchRunner):
    ...
    def _get_custom_metrics(self, state, next_state, env, planner_i=None, planner_j=None):
        ...
        dist = max(abs(state.uav_pos[0] - state.target_pos[0]),
                   abs(state.uav_pos[1] - state.target_pos[1]))
        
        # Manhattan distance
        manhattan = abs(state.uav_pos[0] - state.target_pos[0]) + \
                    abs(state.uav_pos[1] - state.target_pos[1])

        metrics = {
            "distance_chebyshev": dist,
            "distance_manhattan": manhattan,
            "uav_pos": f"{state.uav_pos}",
            "target_pos": f"{state.target_pos}",
            "is_same_row": state.uav_pos[0] == state.target_pos[0],
            "is_same_col": state.uav_pos[1] == state.target_pos[1],
            "is_caught": state.uav_pos == state.target_pos
        }
        ...
        if planner_i and hasattr(planner_i, 'root') and planner_i.root.belief_particles:
            ...
            if pos_counts:
                ...
                metrics["uav_belief_target_pos_mode_prob"] = pos_counts[most_likely_t_pos] / len(particles)
