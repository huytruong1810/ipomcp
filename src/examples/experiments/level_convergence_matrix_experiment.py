...
def plot_payoff_heatmaps(
    payoff_i: np.ndarray, payoff_j: np.ndarray, levels: List[int], out_dir: str
):
    ...
    sns.heatmap(
        ...
        xticklabels=labels,
        yticklabels=labels,
        cbar_kws={"label": "Mean Cum. Reward $R_i$"},
    )
    ...
    sns.heatmap(
        ...
        xticklabels=labels,
        yticklabels=labels,
        cbar_kws={"label": "Mean Cum. Reward $R_j$"},
    )
    ...
    sns.heatmap(
        ...
        xticklabels=labels,
        yticklabels=labels,
        cbar_kws={"label": "Joint Reward $R_i + R_j$"},
    )
...
def find_nash_equilibria(
    payoff_i: np.ndarray, payoff_j: np.ndarray, levels: List[int]
) -> Dict[str, Any]:
...
def run_payoff_matrix_experiment(
    max_level: int = 4,
    n_trials: int = 50,
    max_steps: int = 20,
    planning_depth: int = 5,
    ...
    logger.info(
        f"=== STARTING EMPIRICAL PAYOFF MATRIX BENCHMARK (Levels 0..{max_level}, N={n_trials}, T={max_steps}, Depth={planning_depth}) ==="
    )
    ...
    for i_idx, m in enumerate(levels):
        for j_idx, n in enumerate(levels):
            ...
            logger.info(
                f"[{cell_count}/{total_cells}] Evaluating Cell: Agent I Level {m} vs Agent J Level {n}..."
            )
            ...
            if not df.empty:
                ...
                logger.info(
                    f"  -> Finished Cell L{m} vs L{n}: Mean R_i = {r_i_mean:+.3f} (SEM={r_i_sem:.3f}), Mean R_j = {r_j_mean:+.3f}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Maximum reasoning level to evaluate (0..max_level)",
    )
