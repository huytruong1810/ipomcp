...
1. Deep Hierarchy Prior Benchmark (10 conditions x 200 trials = 2,000 episodes)
2. Apples-to-Apples Doshi-Gmytrasiewicz RTS vs I-POMCP (7 conditions x 200 trials = 1,400 episodes)
3. Full 6x6 Strategy-Level Payoff Matrix (36 cells x 200 trials = 7,200 episodes)
"""
...
def run_all(n_trials: int = 200, max_steps: int = 20, run_suite: str = "all", resume_dir: Optional[str] = None):
    start_time = time.time()
    logger.info(f"================================================================================")
    logger.info(f"=== MASTER LARGE-SCALE BENCHMARK SUITE: N={n_trials} TRIALS, T={max_steps} STEPS ===")
    logger.info(f"================================================================================")

    # Suite 1: Deep Hierarchy Prior Benchmark
    if run_suite in ["all", "prior"]:
        logger.info(f"\n>>> [1/3] LAUNCHING DEEP HIERARCHY PRIOR BENCHMARK (10 Conditions x {n_trials} Trials, T={max_steps}) <<<")
        t0 = time.time()
        run_deep_prior_experiment(n_trials=n_trials, max_steps=max_steps, planning_depth=5, resume_dir=resume_dir)
    ...
    if run_suite in ["all", "oracle"]:
        ...
        t0 = time.time()
        run_apples_to_apples_benchmark(n_trials=n_trials, max_steps=max_steps, planning_depth=3, resume_dir=resume_dir)
        logger.info(f">>> [2/3] Apples-to-Apples Oracle Benchmark Completed in {(time.time() - t0)/60:.1f} minutes.")

    # Suite 3: Full 6x6 Strategy Level Payoff Matrix
    if run_suite in ["all", "matrix"]:
        logger.info(f"\n>>> [3/3] LAUNCHING FULL 6x6 STRATEGY LEVEL PAYOFF MATRIX (36 Cells x {n_trials} Trials, T={max_steps}) <<<")
        t0 = time.time()
        run_payoff_matrix_experiment(max_level=5, n_trials=n_trials, max_steps=max_steps, planning_depth=5, resume_dir=resume_dir)
        logger.info(f">>> [3/3] Full 6x6 Payoff Matrix Benchmark Completed in {(time.time() - t0)/60:.1f} minutes.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=20, help="Environment steps per trial (default: 20)")
