...
class WumpusVisualizer:
    ...
    def print_grid(state: WumpusState):
        ...
        # Status Bar
        status = []
        if state.has_gold: status.append("[VICTORY: GOLD FOUND]")
        if state.has_arrow: status.append("[ARROW]")
        if not state.human_alive: status.append("!!! DIED !!!")
        if not state.wumpus_alive: status.append("(Wumpus Dead)")

        print("Status: " + " ".join(status))

    @staticmethod
    def _get_dir_char(orientation: int) -> str:
        if orientation == NORTH: return '^'
        if orientation == EAST: return '>'
        if orientation == SOUTH: return 'v'
        if orientation == WEST: return '<'
        return '?'