"""Interactive diagnostics for genuine batch records and nested-belief snapshots.

Action values in CSV use JSON. Cumulative-return plots retain absorbing terminal
rows so early failures are not silently removed from the trial population.
Hierarchical graphics depict the approximate represented beliefs, not verified
Bayesian posteriors. See docs/THEORY.md before interpreting level trajectories.
"""
...
def plot_all_metrics(
    df: pd.DataFrame, agent_labels=None, title_prefix: str = "", save_dir: str = None
):
...
def _plot_opponent_level_belief(df, prefix, save_dir):
    ...
    fig.update_layout(
        title=f"{prefix} - Opponent Level Belief Progression over Time",
        xaxis_title="Step (t=0 is Initial Prior)",
    ...
    fig_area.update_layout(
        title=f"{prefix} - Opponent Level Belief Composition (Stacked Area)",
        xaxis_title="Step (t=0 is Initial Prior)",
        yaxis_title="Belief Probability Fraction",
...
def plot_nested_belief_sunburst(
    nested_data: Dict[str, Any],
    title: str = "Nested Mental Model Belief Hierarchy",
    save_dir: Optional[str] = None,
    filename: str = "nested_belief_sunburst",
) -> go.Figure:
    ...
    colors = [
        LEVEL_COLORS.get(lvl, "#cbd5e1")
        for lvl in nested_data.get("levels", [0] * len(nested_data["ids"]))
    ]
...
def plot_episode_sunburst_slider(
    snapshots_by_step: Dict[int, Dict[str, Any]],
    title_prefix: str = "Episode Progression",
    save_dir: Optional[str] = None,
    filename: str = "nested_belief_sunburst_animated",
) -> go.Figure:
    """
        ...
        for idx, nid in enumerate(snap["ids"]):
            if nid not in canonical_meta:
                ...
                canonical_meta[nid] = {
                    ...
                    "level": snap.get("levels", [0])[idx]
                    if "levels" in snap and idx < len(snap["levels"])
                    else 0,
                }
    ...
    canonical_colors = [
        LEVEL_COLORS.get(canonical_meta[nid]["level"], "#cbd5e1") for nid in canonical_order
    ]

    def _extract_aligned_frame_data(
        ...
        for nid in canonical_order:
            if nid in step_id_map:
                ...
                hovers.append(
                    snap["hover_texts"][orig_idx]
                    if "hover_texts" in snap and orig_idx < len(snap["hover_texts"])
                    else ""
                )
            else:
                ...
                vals.append(0.0)
                hovers.append(
                    f"<b>{meta['label']}</b><br>Conditional Belief: 0.0%<br>Joint Mass: 0.000<br>Particles: 0"
                )
        return vals, hovers
    ...
    fig.update_layout(
        title=dict(
            text=f"{title_prefix}: Multi-Level Nested Belief Sunburst (Scrub Timestep)",
            font=dict(size=18),
        ),
...
def _plot_comparative_reward(df, labels, prefix, save_dir):
    # Include terminal outcomes to avoid conditioning returns on survival.
    grouped = (
        df.groupby("step")
        .agg({"cum_reward_i": ["mean", "std"], "cum_reward_j": ["mean", "std"]})
        .reset_index()
    )
    ...
    fig.update_layout(
        title=f"{prefix} - Cumulative Reward over Time",
        xaxis_title="Step",
...
def _plot_reward_vs_time(df, labels, prefix, save_dir):
    if "planning_time_i" not in df.columns:
        return

    df_active = df[df["status"] == "Active"].copy()
    time_col = "planning_time_i"

    df_active["cum_time"] = df_active.groupby("trial")[time_col].cumsum()
    df_active["time_bucket"] = df_active["cum_time"].round(1)

    df_active = (
        df_active.sort_values(["trial", "step"])
        .groupby(["trial", "time_bucket"], as_index=False)
        .tail(1)
    )
    grouped = (
        df_active.groupby("time_bucket")
        .agg({"cum_reward_i": "mean", "cum_reward_j": "mean", "trial": "count"})
        .reset_index()
    )
    ...
    for agent_key, label in labels.items():
        ...
        if col in grouped.columns:
            fig.add_trace(
                go.Scatter(x=grouped["time_bucket"], y=grouped[col], mode="lines", name=label)
            )

    fig.update_layout(
        title=f"{prefix} - Compute-Normalized Reward",
        ...
        if col in df.columns:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=grouped[col],
                    mode="lines",
                    name=f"{label} Active Particles",
                )
            )

    fig.update_layout(
        title=f"{prefix} - Particle Filter Health",
        xaxis_title="Step",
...
def _plot_action_values(df, labels, prefix, save_dir):
    ...
    Reads action-value dictionaries from their JSON representation.
    """
    df_active = df[df["status"] == "Active"]
    ...
    for agent_key, label in labels.items():
        ...
        for _, row in df_active.iterrows():
            ...
            # Deserialize from string if loaded via cold CSV
            ...
            if val and isinstance(val, dict):
                for act, v in val.items():
                    expanded.append(
                        {"step": row["step"], "action": str(act), "value": v, "trial": row["trial"]}
                    )
...
def _plot_distance(df, prefix, save_dir, col="distance"):
    ...
    fig.update_layout(
        title=f"{prefix} - Inter-Agent Distance over Time",
        xaxis_title="Step",
...
def _plot_survival(df, prefix, save_dir):
    ...
    fig.update_layout(
        title=f"{prefix} - Trial Survival Rate",
        xaxis_title="Step",
        yaxis_title="Proportion Active",
        yaxis=dict(range=[0, 1.05]),
    )
