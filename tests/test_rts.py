...
def test_level1_rts_planner_creation_and_action_values():
    ...
    rts_l1 = boot.create_level1_rts_solver(
        "i", env, ["j"], n_particles=30, max_depth=2, obs_branching=2
    )
...
def test_general_create_rts_solver_interface():
    ...
    boot.create_rts_solver("i", 0, env, ["j"])
...
class SimpleTigerRTSBatchRunner(GenericBatchRunner):
    def _setup_domain(self):
        ...
        planner_i = boot.create_rts_solver(
            "i",
            1,
            env,
            ["j"],
            n_particles=20,
            config=RTSConfig(max_depth=2, obs_branching=2, num_particles=20),
        )
