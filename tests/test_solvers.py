...
def test_rts_planner_creation():
    ...
    rts = boot.create_level2_rts_solver(
        "i",
        env,
        ["j"],
...
def test_recursive_arbitrary_level_bootstrapper_level3():
    ...
    sample_sub_l1_particle = [
        p for p in sample_l2_root.belief_particles if p.models["i"][0].level == 1
    ][0]
...
def test_recursive_bootstrapper_level4_and_level5():
    ...
    l5_planner = boot.create_solver(
        agent_id="i", level=5, model=env, other_agent_ids=["j"], n_particles=200, config=cfg
    )
...
def test_solver_bank_isolation():
    ...
    planner_i = boot_i.create_level2_solver(
        "i", env, ["j"], l1_probability=1.0, n_particles=50, config=cfg
    )
